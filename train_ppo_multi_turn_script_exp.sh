...
# Runs grl/train_ppo_multi_turn_example_exp.py and logs output to cache/
...
PY_SCRIPT="$PROJECT_ROOT/grl/train_ppo_multi_turn_example_exp.py"
if [ ! -f "$PY_SCRIPT" ]; then
  echo "Error: Experimental Python script not found: $PY_SCRIPT" | tee -a "$LOG_FILE"
  exit 1
fi
